...
function validateAssets(assets, errors) {
  ...
  return assets.map((asset, index) => {
    ...
    if (typeof asset !== "object" || asset === null || Array.isArray(asset)) {
      errors.push({ field: path, message: "Asset must be an object" });
      return null;
    }
    ...
    if (typeof asset.type !== "string" || asset.type.trim().length === 0) {
      errors.push({ field: `${path}.type`, message: "Asset type is required" });
      hasError = true;
    } else {
    ...
    if (typeof asset.url !== "string" || asset.url.trim().length === 0) {
      errors.push({ field: `${path}.url`, message: "Asset url is required" });
      hasError = true;
    } else {
...
function validateTargets(targets, errors) {
  ...
  return targets.map((target, index) => {
    ...
    if (typeof target !== "object" || target === null || Array.isArray(target)) {
      errors.push({ field: path, message: "Target must be an object" });
      return null;
    }
    ...
    if (typeof target.platform !== "string" || target.platform.trim().length === 0) {
      errors.push({ field: `${path}.platform`, message: "Target platform is required" });
      hasError = true;
    } else {
    ...
    if (target.scheduledAt === undefined) {
      errors.push({ field: `${path}.scheduledAt`, message: "Target scheduledAt is required" });
      hasError = true;
    } else if (!isIsoDate(target.scheduledAt)) {
      errors.push({
        field: `${path}.scheduledAt`,
    ...
    if (target.metadata !== undefined) {
      if (target.metadata === null) {
        ...
        normalized.metadata = target.metadata;
      }
    }
...
function validateContentPayload(payload = {}, { partial = false } = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(payload, "userId")) {
    ...
    } else if (typeof payload.userId === "string" && payload.userId.trim() !== "" && Number.isInteger(Number(payload.userId))) {
  ...
  if (!partial) {
    const processedAssets = validateAssets(payload.assets || [], errors);
    data.assets = processedAssets;
    const processedTargets = validateTargets(payload.targets || [], errors);
